...
**সূত্র**
a. `b^-1 mod M` এ M যদি মৌলিক সংখ্যা হয় তাইলে `ইনভার্স মডিউলো অফ ( b mod M )` = `b^-1 mod M = b^(M-2) mod M`
...
so b ^-1^ mod M = b^M-2^ mod M `if M is prime`
or b ^-1^ mod M = b<sup>ϕ(m) - 2</sup> mod M `if b, M are co-prime`
